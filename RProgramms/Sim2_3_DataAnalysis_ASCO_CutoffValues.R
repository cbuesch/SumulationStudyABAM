################################################################################
#########       Calculating ASCO cutoff values of Simulation 2         #########
################################################################################ 
...
path_load <- tibble(
  Scenario = factor(
    c("Scen1", "Scen2", "Scen3WEIB",  "Scen3GOMP", "Scen4")
  ),
  path_load_data = NA,
  path_load_ana  = NA)
                   ...
                   .combine = rbind) %dopar% {
                     ...
                     df.raw.list <- apply(as.matrix(parameters), MARGIN=1, FUN=function(x){
                       rbind(readRDS(paste0(path_load$path_load_ana[i],
                               ...
                               select(beta, p.C, r, med.C, shape.C.T, HR.var,
                                      designHR, accrual, fu,
